...
Element.Methods = {
    visible: function (a) {
        ...
        var c = d || b || a;
        var f = (function () {
            var g = document.createElement("script"), i = false;
            try {
...
Element.extend = (function () {
    function c(g) {
        if (typeof window.Element != "undefined") {
            ...
            if (i) {
                var k = "_" + (Math.random() + "").slice(2), h = document.createElement(g);
                i[k] = "x";
                var j = (h[k] !== "x");
                delete i[k];
                h = null;
...
(function () {
    ...
    function o(F, G, v) {
        ...
        if (/\d/.test(F) && y && y.runtimeStyle && !(A && w)) {
            var u = y.style.left, E = y.runtimeStyle.left;
        ...
        initialize: function ($super, v, u) {
            ...
            if (u && (w === u)) {
                ...
                if (B === "absolute" || B === "fixed") {
                    ...
                    var C = y.parentNode, z = $(C).getLayout();
    ...
    Object.extend(Element.Layout, {
        ...
        COMPUTATIONS: {
            height: function (w) {
                ...
                var v = this.get("width"), w = this.get("padding-left"), x = this.get("padding-right");
                return v + w + x
                ...
                var u = this.get("border-box-height"), w = this.get("margin-top"), x = this.get("margin-bottom");
                if (u <= 0) {
                    return 0
                }
                return u + w + x
                ...
                var x = u.positionedOffset(), v = u.getOffsetParent(), w = v.measure("height");
...
(function () {
    ...
    b.filter = function (B, A, E, u) {
        var t = B, G = [], y = A, w, e, x = A && A[0] && p(A[0]);
        while (B && A.length) {
    ...
    var f = b.selectors = {
        ...
        relative: {
            "+": function (y, e, x) {
                var v = typeof e === "string", z = v && !/\W/.test(e), w = v && !z;
                if (z && !x) {
                    e = e.toUpperCase()
                }
        ...
        filter: {
            PSEUDO: function (x, t, u, y) {
                ...
                var t = u[1], e = f.attrHandle[t] ? f.attrHandle[t](w) : w[t] != null ? w[t] : w.getAttribute(t), x = e + "", v = u[2], s = u[4];
...
(function () {
    ...
    function c(G) {
        var F = document.documentElement, E = document.body || {scrollLeft: 0};
        return G.pageX || (G.clientX + (F.scrollLeft || E.scrollLeft) - (F.clientLeft || 0))
    }

    function b(G) {
        var F = document.documentElement, E = document.body || {scrollTop: 0};
...
var TimeTracker = {
    measure: function (c, a) {
        var b = (new Date()).getTime();
        var e = b - c;
        new Ajax.Request("/log/measure", {
            method: "post",
            parameters: {requestUrl: window.location.href, pageLoadTime: e, userAgent: navigator.userAgent}
        });
        if (typeof _gaq != "undefined") {
            var d = a ? "fullpage_millis" : "fullpage_millis_anon";
            _gaq.push(["_trackEvent", "timing", d, window.location.pathname, e])
        }
    }
};
var Rounder = function () {
    ...
    var g = function (l, k, w, E, D, v) {
        ...
        for (A = 1; A <= D; A++) {
            var p, q, o, n;
            q = Math.sqrt(1 - d(1 - A / D)) * E;
            var m = E - Math.ceil(q);
            var t = Math.floor(u);
            var C = E - m - t;
            var s = document.createElement("div");
            var r = B;
            s.style.margin = "0px " + m + "px";
            s.style.height = "1px";
            ...
            for (z = 1; z <= C; z++) {
                if (z == 1) {
                    ...
                    if (z == C) {
                        o = Math.sqrt(1 - d((E - m - z + 1) / E)) * D;
                        p = 1 - (1 - (o - (D - A))) * (1 - (u - t)) * 0.5
                    } else {
                        n = Math.sqrt(1 - d((E - m - z) / E)) * D;
                        o = Math.sqrt(1 - d((E - m - z + 1) / E)) * D;
                        p = ((o + n) * 0.5) - (D - A)
                    }
                }
                s.style.backgroundColor = f(k, w, p);
                if (v) {
                    r.appendChild(s)
                } else {
                ...
                s.style.height = "1px";
                s.style.overflow = "hidden";
                s.style.margin = "0px 1px"
            }
            s.style.backgroundColor = w;
            if (v) {
                r.appendChild(s)
            } else {
                r.insertBefore(s, r.firstChild)
    ...
    var a = function (m) {
        ...
        var n = l.exec(m);
        for (var j = 1; j < 4; j++) {
            k += ("0" + parseInt(n[j]).toString(16)).slice(-2)
        }
        return "#" + k
    };
    var d = function (i) {
    ...
    return {
        init: function (i) {
            ...
            $$(".roundedrect").each(function (j) {
                Rounder.addCorners(j, 2, 2)
            });
            if (!i) {
                i = "cbb"
            }
            ...
            if ($(n).hasClassName("rounded-done")) {
                return
            }
            var m = e(n);
